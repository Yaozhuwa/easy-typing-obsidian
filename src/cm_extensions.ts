...
import {
  ...
  removeAllTabstops,
  isInsideCurTabstop,
} from "./tabstops_state_field";
import {
  getCodeBlockInfoInPos,
  isCodeBlockInPos,
  detectRuleScope,
} from "./syntax";
    ...
    (tr: Transaction): TransactionSpec | readonly TransactionSpec[] => {
      ...
      tr.changes.iterChanges((fromA, toA, fromB, toB, inserted) => {
        ...
        if (
          ...
          let base_indent_num = getCodeBlockInfoInPos(
            tr.startState,
            fromA,
          )?.indent;
          let base_indent =
          ...
          if (inserted_lines.length > 1) {
            ...
            let adjusted_lines = inserted_lines.map(
        ...
        if (
          ...
          if (listMatch || quoteMatch) {
            ...
            if (paste_list && listMatch) {
              adjusted_lines = inserted_lines.map(
                (line: string, index: number) => {
                  let trimmed_line = line.substring(min_indent_space);
                  trimmed_line = trimmed_line.replace(
                    /[\t]/g,
  ...
  return EditorView.updateListener.of((update: ViewUpdate) => {
    ...
    if (ctx.compose_need_handle) {
      ...
      if (
        ...
        cursor.anchor > ctx.compose_begin_pos
      ) {
        if (tryProcessInput(ctx, update, ctx.compose_begin_pos, cursor.anchor))
          return;
      }
    }
    ...
    if (
      ...
      if (
        ...
        getPosLineType(update.view.state, toB) === LineType.text
      ) {
